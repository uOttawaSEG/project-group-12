...
public class TeacherRegistrationActivity extends AppCompatActivity{
    ...
    // initalizing the views when clikcking
    ...
    // logic to see if the teacher registration is succesful
    private void attemptRegistration() {
        ...
        if (validInput(firstName, lastName, email, password, phoneNumber, degree, courses)) {
            // make a pop up message to show registration succesful
    ...
    // most of the logic would be the same from the student registration class except for the last two
        ...
        if (!android.util.Patterns.EMAIL_ADDRESS.matcher(email).matches()) {
            edtTeacherEmail.setError("Please enter a valid email adress");
            return false;
        }
